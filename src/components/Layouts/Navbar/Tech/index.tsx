...
import techLogoDark from "../../../../assets/images/tech/tech-logo-dark.png";
...
export default function TechNavbar() {
  ...
  const renderMenuItem = (item: NavMenuItem, isMobile: boolean = false) => (
    <Link
      href={item.href}
      className={`${isMobile ? "block relative px-4 py-2 text-gray-600 dark:text-gray-400" : "relative flex items-center gap-3 px-5 py-2 text-black"} font-medium text-[18px] transition-all duration-300 hover:text-[#592da8] group/underline hover:translate-x-2`}
    >
      <span
        className={`before:content-[''] before:absolute ${isMobile ? "before:left-2" : "before:left-3"} before:top-1/2 before:h-0 before:w-[3px] before:bg-[#592da8] before:-translate-y-1/2 before:rounded-full before:transition-all before:duration-300 group-hover/underline:before:h-5`}
      ></span>
      {item.label}
    </Link>
  );
  ...
  return (
    <>
      {/* Tech Header */}
      <nav className="h-18 transition-all duration-500 ease-out absolute w-full top-0 left-0 z-50 md:px-15 lg:mt-7 mt-4 px-4">
        <div className="md:px-6 px-2 bg-[#fff] dark:bg-[#000] border relative flex items-center justify-between mx-auto h-full py-9">
          {/* Offset background effect */}
          <div className="absolute bg-black dark:bg-white top-[10px] ltr:left-[10px] rtl:right-[10px] w-full h-full -z-10"></div>
          ...
          <div>
            <Link href="/">
              <Image
                src={techLogoDark.src}
                alt="logo"
                width={techLogoDark.width}
                height={techLogoDark.height}
          ...
          <ul className="hidden items-center gap-12 font-medium xl:flex">
            {navigationData.menus.map((menu: NavMenu, index: number) => (
              <li key={index} className="group relative">
                ...
                <a
                  href="#!"
                  className="flex items-center gap-1 h-22 text-black dark:text-white text-[18px] font-medium hover:text-[#592da8] transition-all duration-300"
                >
                  {menu.label}
                  {/* PLUS icon */}
                ...
                {/* MEGA MENU for Home */}
                {menu.megaMenu && (
                  <div className="dropdown-menu absolute top-full ltr:-translate-x-1/5 rtl:translate-x-1/5 w-[950px] bg-white border-b-4 ltr:border-r-4 rtl:border-l-4 border-black shadow-xl text-black opacity-0 invisible scale-y-0 origin-top group-hover:opacity-100 group-hover:visible group-hover:scale-y-100 transition-all duration-500 ease-out z-50">
                            ...
                            (item: HomeMegaMenuItem, itemIndex: number) => (
                              ...
                              >
                                ...
                                <p className="mt-3 text-center">
                                  <Link
                                    href={item.href}
                                    className="relative flex items-center gap-3 px-5 py-2 font-medium text-[18px] transition-all duration-300 hover:text-[#592da8] group/underline hover:translate-x-2"
                                  >
                                    <span className="before:content-[''] before:absolute before:left-3 before:top-1/2 before:h-0 before:w-[3px] before:bg-[#592da8] before:-translate-y-1/2 before:rounded-full before:transition-all before:duration-300 group-hover/underline:before:h-5"></span>
                                    {item.label}
                                  </Link>
                                </p>
                              </div>
                ...
                {menu.items && !menu.megaMenu && (
                  <div className="dropdown-menu absolute top-full ltr:border-r-4 border-b-4 rtl:border-l-4 border-black text-black w-48 bg-white opacity-0 invisible group-hover:opacity-100 group-hover:scale-y-100 group-hover:visible scale-y-0 origin-top transition-all duration-500 ease-out z-50">
                    <ul className="py-3">
                      {menu.items.map(
